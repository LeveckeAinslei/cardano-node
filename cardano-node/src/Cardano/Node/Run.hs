...
handleSimpleNode blockType runP p2pMode tracers nc onKernel = do
 ...
 where
  ...
  customizeChainSyncTimeout = case (ncChainSyncCanAwaitTimeout nc, ncChainSyncIntersectTimeout nc, ncChainSyncMustReplyTimeout nc, ncChainSyncIdleTimeout nc) of
    (NoTimeoutOverride, NoTimeoutOverride, NoTimeoutOverride, NoTimeoutOverride) -> Nothing
    _ -> Just $ do
      cst <- stdChainSyncTimeout
      let cst1 = case ncChainSyncCanAwaitTimeout nc of
            TimeoutOverride 0 -> cst { canAwaitTimeout = Nothing }
            TimeoutOverride t -> cst { canAwaitTimeout = Just t }
            _ -> cst
          cst2 = case ncChainSyncIntersectTimeout nc of
            TimeoutOverride 0 -> cst1 { intersectTimeout = Nothing }
            TimeoutOverride t -> cst1 { intersectTimeout = Just t }
            _ -> cst1
          cst3 = case ncChainSyncMustReplyTimeout nc of
            TimeoutOverride 0 -> cst2 { mustReplyTimeout = Nothing }
            TimeoutOverride t -> cst2 { mustReplyTimeout = Just t }
            _ -> cst2
          cst4 = case ncChainSyncIdleTimeout nc of
            TimeoutOverride 0 -> cst3 { idleTimeout = Nothing }
            TimeoutOverride t -> cst3 { idleTimeout = Just t }
            _ -> cst3
      pure cst4
